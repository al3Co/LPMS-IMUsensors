%% (18-01-2018) This is a script that reads data from a single sensor and plot its orientation
close all
clear
clc
...
if ( ~lpSensor1.connect(COMPort, baudrate) )
...
while double(get(gcf,'CurrentCharacter'))~=27
    ...
    if (~isempty(d1))
        ...
        data(3) = rollS1; % yaw or roll, depending the position of the sensor on the arm-frame
    end
    if (~isempty(d2))
        [yawS2, pitchS2, rollS2] = quat2angle(d2.quat);
        data(4) = rollS2; % yaw or roll, depending the position of the sensor on the arm-frame
    end
    funcDrawSensor(data);
end
